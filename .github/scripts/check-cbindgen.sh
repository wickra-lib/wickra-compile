...
c_header="bindings/c/include/wickra_compile.h"
go_header="bindings/go/include/wickra_compile.h"
...
cbindgen --config bindings/c/cbindgen.toml --crate wickra-compile-c --output "$tmp" --quiet

if ! diff -u "$c_header" "$tmp"; then
    echo ""
    echo "ERROR: $c_header is out of sync with cbindgen output."
    echo "Regenerate it with:"
    echo "  cbindgen --config bindings/c/cbindgen.toml --crate wickra-compile-c --output $c_header"
    exit 1
fi
