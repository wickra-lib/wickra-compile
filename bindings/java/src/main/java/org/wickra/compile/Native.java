...
/** Raw FFM (Panama) downcall surface for the wickra-compile C ABI. */
final class Native {
    private Native() {}
    ...
    static final MethodHandle NEW =
            handle("wickra_compile_new", FunctionDescriptor.of(C_PTR));
    static final MethodHandle FREE =
            handle("wickra_compile_free", FunctionDescriptor.ofVoid(C_PTR));
    static final MethodHandle COMMAND =
            handle("wickra_compile_command", FunctionDescriptor.of(C_INT, C_PTR, C_PTR, C_PTR, C_LONG));
    static final MethodHandle ARTIFACT_READ =
            handle("wickra_compile_artifact_read", FunctionDescriptor.of(C_LONG, C_PTR, C_LONG, C_PTR, C_LONG));
    static final MethodHandle VERSION =
            handle("wickra_compile_version", FunctionDescriptor.of(C_PTR));
    ...
    private static SymbolLookup loadLibrary() {
        String libFile = System.mapLibraryName("wickra_compile");
        ...
        throw new UnsatisfiedLinkError("wickra-compile: could not load the native library (" + libFile
    ...
    private static Path extractBundled(String libFile) {
        ...
        try (InputStream in = Native.class.getResourceAsStream(resource)) {
            ...
            Path tmp = Files.createTempFile("wickra-compile-", "-" + libFile);
